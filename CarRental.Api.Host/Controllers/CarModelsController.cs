...
public class CarModelsController(ICarModels carModels, ILogger<CarModelsController> logger) : Controller
{
    /// <summary>
    ...
    [ProducesResponseType(500)]
    public ActionResult<CarModelDto> Read(int id)
    {
        ...
        try
        {
            ...
            if (result == null)
            {
                logger.LogWarning("Car model with id {id} not found", id);
                return NotFound();
            }
        ...
        {
            logger.LogWarning("Car model with id {id} not found: {message}", id, ex.Message);
    ...
    [ProducesResponseType(500)]
    public ActionResult<CarModelDto> Create(CarModelsCreateDto modelDto)
    {
        logger.LogInformation("{method} method of {controller} is called",
